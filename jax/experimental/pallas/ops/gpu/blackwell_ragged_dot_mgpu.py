...
class AxisInfo:
  ...
  def all_axes(self) -> tuple[str, ...]:
    return (self.wg_axis, self.sm_axis) + self.collective_axes
...
def do_matmul(a_gmem,
  ...
  if collective:
    ...
    is_lead_block = True
  wg_idx = lax.axis_index(axis_info.wg_axis)
  ...
  @functools.partial(pl.run_scoped,
      ...
      collective_axes=(axis_info.wg_axis,)
  )
  def _scoped(a_smem, b_smem, acc_tmem, acc_smem,
        ...
        def _memory():
          def _loop_body(ki, _):
            ...
            plgpu.copy_gmem_to_smem(
                ...
                collective_axes=(axis_info.collective_axes[0]
        ...
        def _compute():
          def _loop_body(ki, _):
            ...
            plgpu.tcgen05_mma(
                ...
                accumulate=(ki > 0),
                collective_axis=(axis_info.collective_axes[0]
                                 if collective else None),
            )
    ...
    def _():
      ...
      cluster0_block_size = lax.max(
          lax.min(block_tile_m - group_info.start_within_block,
                  group_info.actual_size), jnp.int32(0))
      block_local_size = lax.select(cluster_idx == 0,
        # cluster_idx=0
        cluster0_block_size,
        # cluster_idx=1
        group_info.actual_size - cluster0_block_size
      )
...
def ragged_dot_kernel(a, b, group_sizes, config: TuningConfig):
  ...
  def kernel(a_gmem, b_gmem, group_sizes_gmem, out_gmem):
    ...
    if config.grid_tile_n is not None:
      grid_tiling = (grid[0], config.grid_tile_n) + grid[2:]
    else:
      grid_tiling = None
    collective_axes = (axis_info.sm_axis,) + axis_info.collective_axes
  ...
  f = plgpu.kernel(
      ...
      grid_names=(axis_info.sm_axis,),
      num_threads=2,
      thread_name=axis_info.wg_axis,
      cluster_names=axis_info.collective_axes,
